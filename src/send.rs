//! `notify` 子命令:本机直接发一条标题通知,不经 HTTP——安装前手测弹窗/兜底用。
//! 仅支持标题(-t);默认走弹窗(点击关闭后进程退出),-f 强制走系统通知兜底。
//! `close` 子命令:关闭当前弹窗(经运行中服务的 /close,幂等)。
...
pub fn run(cfg: &Config, title: String, fallback: bool) {
    let req = crate::api::NotifyRequest { title, body: None };
    ...
    let posted = slint::invoke_from_event_loop(move || notify::popup::spawn(&title, "", true));
    if posted.is_err() {
        eprintln!("弹窗投递失败");
        std::process::exit(1);
    }
